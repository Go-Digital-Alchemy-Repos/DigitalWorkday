...
import {
  ...
  type ClientStageAutomationRule, type InsertClientStageAutomationRule,
  type ClientStageAutomationEvent, type InsertClientStageAutomationEvent,
...
export interface IStorage {
  ...
  getBatchTasksWithRelations(taskIds: string[]): Promise<TaskWithRelations[]>;
  ...
  getAutomationRulesByTenant(tenantId: string, enabledOnly?: boolean): Promise<ClientStageAutomationRule[]>;
  getAutomationRule(id: string, tenantId: string): Promise<ClientStageAutomationRule | undefined>;
  createAutomationRule(rule: InsertClientStageAutomationRule): Promise<ClientStageAutomationRule>;
  updateAutomationRule(id: string, tenantId: string, updates: Partial<InsertClientStageAutomationRule>): Promise<ClientStageAutomationRule | undefined>;
  deleteAutomationRule(id: string, tenantId: string): Promise<boolean>;
  createAutomationEvent(event: InsertClientStageAutomationEvent): Promise<ClientStageAutomationEvent>;
  getAutomationEvents(tenantId: string, limit?: number): Promise<ClientStageAutomationEvent[]>;
...
export class DatabaseStorage implements IStorage {
  ...
  async getBatchTasksWithRelations(taskIds: string[]): Promise<TaskWithRelations[]> {
    ...
    const chunks = chunkArray(taskIds, 500);
    const taskChunks = await Promise.all(
      chunks.map(chunk => db.select().from(tasks).where(inArray(tasks.id, chunk)))
    );
    const taskList = taskChunks.flat();

    const [assigneeRows, watcherRows, tagJoinRows, subtaskRows, childTaskRows] = await Promise.all([
      ...
      db.select({ ta: taskTags, tag: tags })
        .from(taskTags)
        .leftJoin(tags, eq(taskTags.tagId, tags.id))
        .where(inArray(taskTags.taskId, taskIds)),
      db.select().from(subtasks).where(inArray(subtasks.taskId, taskIds)).orderBy(asc(subtasks.orderIndex)),
      db.select().from(tasks).where(
        and(inArray(tasks.parentTaskId, taskIds), isNotNull(tasks.parentTaskId))
      ).orderBy(asc(tasks.orderIndex)),
    ]);
    ...
    const allSectionIds = [...new Set(taskList.map(t => t.sectionId).filter(Boolean) as string[])];
    const allProjectIds = [...new Set(taskList.map(t => t.projectId).filter(Boolean) as string[])];

    const [sectionList, projectList] = await Promise.all([
      allSectionIds.length > 0 ? db.select().from(sections).where(inArray(sections.id, allSectionIds)) : Promise.resolve([]),
      allProjectIds.length > 0 ? db.select().from(projects).where(inArray(projects.id, allProjectIds)) : Promise.resolve([]),
    ]);
    ...
    const watchersByTask = new Map<string, typeof watcherRows>();
    ...
    const tagsByTask = new Map<string, typeof tagJoinRows>();
    for (const r of tagJoinRows) {
      const arr = tagsByTask.get(r.ta.taskId) ?? [];
      arr.push(r);
      tagsByTask.set(r.ta.taskId, arr);
    }
    ...
    const childsByTask = new Map<string, typeof childTaskRows>();
    for (const ct of childTaskRows) {
      if (!ct.parentTaskId) continue;
      const arr = childsByTask.get(ct.parentTaskId) ?? [];
      arr.push(ct);
      childsByTask.set(ct.parentTaskId, arr);
    }
    ...
    return taskIds
      ...
      .map(task => ({
        ...
        watchers: (watchersByTask.get(task!.id) ?? []).map(w => ({
          ...w,
        ...
        childTasks: (childsByTask.get(task!.id) ?? []).map(ct => ({
          ...ct,
          assignees: [],
          watchers: [],
          tags: [],
          subtasks: [],
          childTasks: [],
          section: ct.sectionId ? sectionMap.get(ct.sectionId) : undefined,
          project: ct.projectId ? projectMap.get(ct.projectId) : undefined,
        })),
        section: task!.sectionId ? sectionMap.get(task!.sectionId) : undefined,
  ...
  async getTasksByProject(projectId: string): Promise<TaskWithRelations[]> {
    ...
    const batchResult = await this.getBatchTasksWithRelations(taskIds);
    return taskIds.map(id => batchResult.find(t => t.id === id)).filter(Boolean) as TaskWithRelations[];
  }
  ...
  async getClientsByWorkspace(workspaceId: string): Promise<ClientWithContacts[]> {
    ...
    const [allContacts, allProjects] = await Promise.all([
      db.select().from(clientContacts).where(inArray(clientContacts.clientId, clientIds)),
    ...
    for (const contact of allContacts) {
      const arr = contactsByClient.get(contact.clientId) ?? [];
      arr.push(contact);
      contactsByClient.set(contact.clientId, arr);
    }
  ...
  async getAutomationRulesByTenant(tenantId: string, enabledOnly = false): Promise<ClientStageAutomationRule[]> {
    const conditions = [eq(clientStageAutomationRules.tenantId, tenantId)];
    if (enabledOnly) {
      conditions.push(eq(clientStageAutomationRules.isEnabled, true));
    }
    return db.select()
      .from(clientStageAutomationRules)
      .where(and(...conditions))
      .orderBy(asc(clientStageAutomationRules.createdAt));
  }

  async getAutomationRule(id: string, tenantId: string): Promise<ClientStageAutomationRule | undefined> {
    const [rule] = await db.select()
      .from(clientStageAutomationRules)
      .where(and(
        eq(clientStageAutomationRules.id, id),
        eq(clientStageAutomationRules.tenantId, tenantId),
      ));
    return rule;
  }

  async createAutomationRule(rule: InsertClientStageAutomationRule): Promise<ClientStageAutomationRule> {
    const [created] = await db.insert(clientStageAutomationRules).values(rule).returning();
    return created;
  }

  async updateAutomationRule(id: string, tenantId: string, updates: Partial<InsertClientStageAutomationRule>): Promise<ClientStageAutomationRule | undefined> {
    const [updated] = await db.update(clientStageAutomationRules)
  ...
  async deleteAutomationRule(id: string, tenantId: string): Promise<boolean> {
    const result = await db.delete(clientStageAutomationRules)
      .where(and(
        eq(clientStageAutomationRules.id, id),
        eq(clientStageAutomationRules.tenantId, tenantId),
      ))
  ...
  async createAutomationEvent(event: InsertClientStageAutomationEvent): Promise<ClientStageAutomationEvent> {
    const [created] = await db.insert(clientStageAutomationEvents).values(event).returning();
    return created;
  }

  async getAutomationEvents(tenantId: string, limit = 50): Promise<ClientStageAutomationEvent[]> {
    return db.select()
      .from(clientStageAutomationEvents)
      .where(eq(clientStageAutomationEvents.tenantId, tenantId))
      .orderBy(desc(clientStageAutomationEvents.createdAt))
      .limit(limit);
  }
  ...
  async getNotificationsByUserPaginated(userId: string, tenantId: string | null, options?: { unreadOnly?: boolean; limit?: number; cursor?: string; typeFilter?: string }): Promise<{ items: Notification[]; nextCursor: string | null; hasMore: boolean }> {
    return notificationsRepo.getNotificationsByUserPaginated(userId, tenantId, options);
  }
  ...
  async dismissNotification(id: string, userId: string, tenantId: string | null): Promise<Notification | undefined> {
    return notificationsRepo.dismissNotification(id, userId, tenantId);
  }
  ...
  async upsertUserUiPreferences(userId: string, tenantId: string | null, prefs: { themeMode?: string | null; themePackId?: string | null; themeAccent?: string | null; sidebarProjectOrder?: string[] | null }): Promise<UserUiPreferences> {
    const now = new Date();
    const [result] = await db
      ...
      .onConflictDoUpdate({
        ...
        set: {
          ...
          ...(prefs.sidebarProjectOrder !== undefined ? { sidebarProjectOrder: prefs.sidebarProjectOrder } : {}),
          updatedAt: now,
        },
      })
      .returning();
