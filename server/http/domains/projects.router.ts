/**
 * Projects Domain Router (Prompt #11 — Slice 1: Projects Core)
 *
 * Migrated from:
 *   - server/routes/projects.router.ts (18 endpoints)
 *
 * Endpoint inventory (18 endpoints):
 *   Project CRUD:
 *     GET    /projects                         — list projects for user (tenant-scoped)
 *     GET    /projects/unassigned              — unassigned projects
 *     GET    /projects/hidden                  — hidden projects for current user
 *     GET    /projects/:id                     — get project by id (tenant-scoped)
 *     POST   /projects                         — create project (with members, client/division validation)
 *     PATCH  /projects/:id                     — update project (with client/division validation, notifications)
 *
 *   Project Members:
 ...
 *     PUT    /projects/:projectId/members      — set all members (bulk replace)
 *
 *   Project Visibility:
 *     POST   /projects/:projectId/hide         — hide project from user's view
 *     DELETE /projects/:projectId/hide         — unhide project
 *     GET    /projects/:projectId/hidden       — check if project is hidden for user
 *
 ...
 *     PATCH  /projects/:projectId/tasks/reorder — reorder tasks within sections
 *
 * skipEnvelope: true — All handlers use res.json() directly with established response shapes.
 * Tenancy-aware: Uses getEffectiveTenantId() with super-user fallback.
 */
...
// ---------------------------------------------------------------------------
// Project Visibility (hide/unhide)
// ---------------------------------------------------------------------------
