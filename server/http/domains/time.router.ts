/**
 * Time Domain Router (Prompt #10)
 *
 * Migrated from:
 *   - server/routes/timeTracking.router.ts (18 endpoints)
 *   - server/routes/timeTracking.ts (7 overlapping timer endpoints, simpler version)
 *
 * Endpoint inventory (18 endpoints):
 *   Timer CRUD:
 *     GET    /timer/current             — get current user's active timer
 *     POST   /timer/start               — start a new timer
 *     POST   /timer/pause               — pause running timer
 *     POST   /timer/resume              — resume paused timer
 *     PATCH  /timer/current             — update timer details
 *     POST   /timer/stop                — stop timer (creates time entry or discards)
 *     DELETE /timer/current             — discard timer without saving
 *
 *   Time Entry CRUD:
 *     GET    /time-entries              — list time entries (workspace, filters)
 *     GET    /time-entries/my           — current user's time entries
 *     GET    /time-entries/my/stats     — personal time stats dashboard
 *     GET    /time-entries/:id          — single time entry
 ...
 *
 *   Reporting:
 *     GET    /time-entries/report/summary  — time tracking summary report
 *     GET    /time-entries/export/csv      — export time entries as CSV
 *
 *   Calendar:
 *     GET    /calendar/events           — unified calendar view (tasks + time entries)
 *     GET    /my-calendar/events        — personal calendar view
 *
 * Invariants confirmed:
 *   - Single active timer per user (conflict 409 if starting while one exists)
 *   - Timer must be running to pause; must be paused to resume
 *   - Stop with discard deletes timer without creating entry
 *   - Stop without discard + zero duration throws 400
 *   - Tenancy-aware: strict/soft/legacy modes supported
 *
