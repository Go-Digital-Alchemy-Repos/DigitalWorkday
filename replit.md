...
- **Cloudflare R2 Storage**: Cloudflare R2 is the exclusive storage provider for all file uploads. Resolution order: tenant R2 → system R2 → environment R2 (CF_R2_* variables). Uses S3-compatible API via AWS SDK. **Image Compression Pipeline**: All image uploads are automatically compressed: (1) Server-side proxy uploads use `imageProcessor.ts` with sharp for resizing/format conversion; (2) Client-side presigned uploads (avatars) use `ImageCropper.tsx` for compression before upload. Compression configs: avatars 400x400/WebP, logos 1200x400, icons 512x512, favicons 64x64, attachments 2000x2000.
