...
- **Guided Tour / Contextual Help System**: Two-phase in-app help system. Frontend module at `client/src/features/guidedTours/`.
  - **Phase 1 — Tour Runtime**: `TourStepOverlay` (portal spotlight + popover, data-testid="tour-step-popover"), tour registry (3 production tours), localStorage persistence, GuidedToursContext store (useReducer), `useGuidedTours` hook (backend sync, route navigation, duplicate-launch guard), `GuidedTourProvider` (wired in App.tsx), `TourStepOverlay`, `GuidanceCenter` sheet, `TourLauncher`, `useTourApi` TanStack Query hooks. Route navigation via `requiredRoute` per step. `data-tour` attrs: home.tsx (home-stat-cards, home-focus-tasks, home-upcoming), projects-dashboard.tsx (projects-create-btn, projects-search, projects-filter-bar), my-tasks.tsx (my-tasks-personal-task-btn, my-tasks-search), my-time.tsx (my-time-start-timer). Backend: `user_guided_tour_preferences` and `user_guided_tour_progress` tables; 7 REST endpoints at `/api/guided-tours/*`. Barrel export: `client/src/features/guidedTours/index.ts`.
  - **Phase 2 — Contextual Hints**: Passive pulsing beacon system. `ContextualHintBeacon` — portal-rendered 10px fixed-position dot with `animate-ping` ring; uses `waitForTarget()` (5s backoff polling) for late-rendered targets; hover/click opens 244px popup card with Lightbulb icon, title, body, "Don't show again" button. `ContextualHintRenderer` — global renderer wired in App.tsx; filters hints by route/role/dismissed-state; max 3 per screen, sorted by priority; suppressed during active tours or when GuidanceCenter is open; hides on viewports < 640px. `hintRegistry.ts` — 8 starter hints (dashboard-stats, dashboard-focus, projects-create, projects-filter, tasks-personal, tasks-search, time-start-timer, chat-entry). `hintPersistence.ts` — localStorage key `dw_hint_dismissed` stores `{hintId: version}` map. Store extended with `dismissedHintVersions` state and `DISMISS_HINT`/`RESET_DISMISSED_HINTS`/`LOAD_DISMISSED_HINTS` actions. GuidanceCenter updated with dismissed hint count + "Re-enable all" button (data-testid="reset-dismissed-hints").
