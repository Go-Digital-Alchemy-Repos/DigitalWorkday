...
- **Client 360 View**: Tabbed CRM profile page (`/clients/:id/360`) behind `CRM_CLIENT_360_ENABLED` feature flag. Tabs: Overview (CRM summary cards, quick actions including "Invite to Portal"), Projects (placeholder), Contacts (list with DataToolbar search, add/edit drawer, mark primary via CRM API), Activity (timeline), Files (DataToolbar search + visibility filter), Notes (rich text editor, notes feed with author + timestamp via CRM API), Reports (client profitability dashboard), Approvals (DataToolbar search + status filter, gated by `CRM_APPROVALS_ENABLED`), Messages (DataToolbar search, gated by `CRM_CLIENT_MESSAGING_ENABLED`). All tabs use `animate-tab-in` CSS animation (200ms fade-in, respects prefers-reduced-motion). Navigation link added to client detail header when flag is enabled. Non-destructive: existing client detail page unchanged. See `docs/CRM/CRM_CLIENT_360.md` and `docs/CRM/CRM_OVERVIEW.md`.
- **Client Profitability Reports**: Admin-only reporting in Client 360 "Reports" tab. API: `GET /api/crm/clients/:clientId/metrics?from=&to=` returns totalHours, billableHours, totalTimeEntries, hoursByProject, hoursByEmployee, recentEntries (up to 100), and revenueEstimate placeholder. UI: summary cards (total/billable/non-billable hours, revenue estimate), stacked bar charts for hours by project and employee (Recharts), time entries table with CSV export, date range filtering. Reuses existing time_entries data aggregated via project→client relationship. Security: tenant-scoped, admin/super-user only.
- **Client Approval Workflows**: `approval_requests` table (migration 0014) for client review/approve workflows. Admin creates approval request tied to client (optionally project/task) → client receives in portal → client approves or requests changes with comment → requester notified via `approval_response` notification type. API: POST/GET `/api/crm/clients/:clientId/approvals`, PATCH `/api/crm/approvals/:id` (client-only), GET `/api/crm/portal/approvals`. Portal page at `/portal/approvals` with filtering by status. Client 360 Overview Quick Actions includes "Request Approval" button. Security: tenant-scoped, PATCH restricted to client role only, status locked after response. Feature-flag gated by `CRM_APPROVALS_ENABLED`.
- **Client Messaging**: `client_conversations` and `client_messages` tables (migration 0015) for client-safe communication separate from internal chat. Conversations scoped by clientId (optionally projectId). Admin/employee can start conversations; clients can reply. API: GET/POST `/api/crm/clients/:clientId/conversations`, GET/POST `/api/crm/conversations/:conversationId/messages`, GET `/api/crm/portal/conversations`. Portal page at `/portal/messages` with conversation list and threaded message view. Client 360 "Messages" tab with inline conversation creation and reply. Feature-flag gated by `CRM_CLIENT_MESSAGING_ENABLED`. Security: tenant-scoped, client users restricted to accessible clients only, closed conversations are read-only.
...
- **Design Token System**: Formal CSS design tokens in `client/src/styles/tokens.css` provide a single source of truth for spacing scale (space-0 through space-16 + semantic aliases page/section/card/inline), typography ramp (display/h1-h4/body/small/caption/overline with line-heights and weights), motion tokens (duration-instant through duration-slower + 5 easing functions), radii, z-index scale, and semantic surface aliases. Tailwind config extends with token-backed fontSize/spacing/transitionDuration/transitionTimingFunction/zIndex utilities. Layout primitives consolidated in `client/src/components/ui-system/` (AppShell, PageHeader, Typography, etc.) with backward-compatible re-exports from `client/src/components/layout/`. See `docs/05-FRONTEND/UI-CONSISTENCY-CHECKLIST.md` for usage rules.
