...
- **Centralized Query Keys**: All React Query keys are defined in `client/src/lib/queryKeys.ts` with a shared `invalidateTaskCaches()` helper for consolidated cache invalidation. Sprint-touched files import from `queryKeys` instead of using hardcoded string arrays. See `docs/performance/sprint-optimization-guide.md` for the full 10-topic performance guide.
