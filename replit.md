...
- **Email Templates**: Customizable email templates managed via Super Admin > System Settings > Email Templates. Table: `email_templates` with tenant-first resolution (tenant override → system default → hardcoded fallback). Default templates seeded on startup: `forgot_password`, `mention_notification`, `invitation`, `task_assignment`, `welcome_email`. Templates use `{{variableName}}` syntax for dynamic content. Service: `server/services/emailTemplates.ts` (render, resolve, CRUD). Defaults: `server/services/emailTemplateDefaults.ts`. API: `GET/PUT /api/v1/system/email-templates/:key`, `POST /preview`, `POST /reset`. Frontend: `client/src/components/settings/email-templates-tab.tsx`. Email outbox (`server/services/emailOutbox.ts`) resolves Mailgun config with 3-tier fallback via `resolveMailgunConfig()`: tenant `tenantIntegrations` → system `tenantIntegrations` (tenantId=NULL) → `system_settings` table columns (`mailgunDomain`, `mailgunFromEmail`, `mailgunRegion`, `mailgunApiKeyEncrypted`). The `system_settings` tier is where Super Admin > Integrations > Mailgun stores its config and is the primary production fallback.
