...
- **Global Branding & Theming**: System-level branding with 9 theme packs across 3 categories (Light: Light, Sand; Dark: Dark, Midnight, Graphite; Funky: The 80's, Vaporwave, Hacker, Sunset). Funky themes support per-theme font overrides (e.g. Orbitron for 80's, Space Mono for Hacker) via `--font-sans` CSS variable token, with automatic reset to Inter when switching away. Configurable via Super Admin settings and user preferences.
