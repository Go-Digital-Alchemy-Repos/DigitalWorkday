...
- **Guided Tour / Contextual Help System**: In-app onboarding and help system. Frontend module at `client/src/features/guidedTours/` with: type definitions, tour registry (3 demo tours), localStorage persistence, GuidedToursContext store (useReducer), `useGuidedTours` hook, `GuidedTourProvider` (wired in App.tsx, gates API calls on auth), `GuidanceCenter` sheet (accessible from user menu "Help & Tours"), `TourLauncher`, `ContextualHint`, and `useTourApi` TanStack Query hooks. Backend: `user_guided_tour_preferences` and `user_guided_tour_progress` tables; 7 REST endpoints at `/api/guided-tours/*` (mounted via `guidedTours.router.ts`). Tour engine uses a no-op adapter pattern — swap `createDriverAdapter()` in `tourEngineAdapter.ts` when Driver.js is installed. Barrel export: `client/src/features/guidedTours/index.ts`.
