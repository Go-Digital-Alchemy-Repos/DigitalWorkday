...
- **Time Entries Cursor Pagination**: `GET /api/time-entries` and `GET /api/time-entries/my` support optional cursor-based pagination via `limit` and `cursor` query params. When `limit` is provided (max 50), response is `{ items, hasMore, nextCursor, totalCount }`. When omitted, returns flat array for backward compatibility. Frontend uses `useInfiniteQuery` with "Load more" for "All Time" and "This Month" filters. Query key `queryKeys.timeEntries.paginated` used for paginated cache.
...
- **Canonical Secret Handling**: All provider-specific secret decryption (AI/OpenAI, Storage/R2, QuickBooks) is consolidated through `TenantIntegrationService`. AI and Storage resolvers use `getIntegrationDetailedSecrets()` (returns ID, status, decrypt-status flags for error discrimination). QuickBooks uses `getIntegrationWithSecrets()`. No inline `JSON.parse(decryptValue(...))` patterns remain in provider resolvers. `IntegrationProvider` type includes `"quickbooks"`.
- **Lightweight Picker Endpoints**: Dedicated `GET /api/v1/pickers/clients`, `/projects`, `/tasks` returning `{id, label}` arrays for dropdown/select UI. Tenant-scoped with private-project/task visibility filters. Used by `useTimeEntryCascade` hook, `StartTimerDrawer`, `GlobalActiveTimer`, and all time-entry dialogs. Query keys in `queryKeys.pickers`. See `docs/architecture/MY_TIME_PERFORMANCE.md`.
- **Observability Hardening**: In-memory `EndpointLatencyTracker` (ring buffer, ~1000 samples per endpoint) computes running p50/p95/p99 latency, avg payload bytes, and avg DB query count per endpoint. Surfaced via `latencyDistribution` on `/api/v1/system/observability`. Performance budgets cover 15 endpoint groups (tasks, clients, projects, time entries, timers, notifications, reports). Unified `[perf:budget]` warnings check latency, payload, and query count dimensions. `requestLogger` includes `payloadBytes` in structured log lines. See `docs/performance/OBSERVABILITY_GUIDE.md`.
- **Platform Hardening & List DTOs (Phase 3)**: All major domains (clients, projects, tasks, time entries) support a `fields=list` query parameter returning lightweight payloads for table/list views. Picker endpoints (`/api/v1/pickers/*`) return `{id, label}` for dropdowns. Query key registry (`queryKeys.ts`) includes `.list` keys for clients and projects with corresponding `invalidateClientCaches`/`invalidateProjectCaches` coverage. Cache invalidation helpers are tenant-namespaced and audit-verified — no tenant isolation gaps. Worker separation via `PROCESS_MODE` env var (`all`/`api`/`worker`) with dedicated `server/worker.ts` entry point. Staging profiling enabled via `config.features.enablePerfProfiling`. See `docs/architecture/PLATFORM_HARDENING.md`.
- **Enterprise Stabilization Sprint Summary**: Full sprint documentation across 4 phases (16 items) covering TypeScript stabilization, CI policy, compiler target, secret handling, DTO strategy, SQL-first aggregation, My Time stats, pagination, list DTOs, targeted cache invalidation, picker endpoints, tenant query keys, canonical endpoints, staging profiling, worker separation, and follow-ups. Regressions found and fixed: `projects.router.ts` budget column mismatch, `pickers.router.ts` nullable tenantId. See `docs/architecture/ENTERPRISE_STABILIZATION_SPRINT.md`.
