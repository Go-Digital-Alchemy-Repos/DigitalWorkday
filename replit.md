...
- **Canonical Secret Handling**: All provider-specific secret decryption (AI/OpenAI, Storage/R2, QuickBooks) is consolidated through `TenantIntegrationService.getIntegrationWithSecrets()`. No inline `JSON.parse(decryptValue(...))` patterns remain in provider resolvers. `IntegrationProvider` type includes `"quickbooks"`.
