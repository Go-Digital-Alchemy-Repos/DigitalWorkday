...
- **Performance Playbook**: Core principles: (1) **Never call per-row relation loaders in a loop** — always use `getBatchTasksWithRelations(ids)` or equivalent `inArray()`-based batch methods. (2) Use `Promise.all` for independent parallel queries. (3) Chunk `inArray` inputs to 500 IDs max. (4) Frontend lists use `staleTime: 30 000–60 000ms`; report/analytics queries use `staleTime: 120 000ms`; real-time queries (presence, timers) stay at `staleTime: 10 000ms`. Key N+1 fixes applied: `getTasksByUser` + `getTasksByProject` now use `getBatchTasksWithRelations` (reduces ~400 queries to ~9 for 50 tasks); `getClientsByTenant` uses `inArray` batch loading (2N → 2 queries). Backend telemetry: set `PERF_TELEMETRY=1` to enable slow-request (>800ms) and slow-query (>500ms) logging.
