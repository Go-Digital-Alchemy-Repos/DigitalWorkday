...
case "$MODE" in
  --build-only) ;;
  --verify) codesign --verify --deep --strict --verbose=2 "$APP_DIR" ;;
  --run) open "$APP_DIR" ;;
  --logs) log stream --level info --predicate 'process == "DigitalWorkday"' ;;
  --telemetry) log stream --style compact --predicate 'subsystem == "ai.digitalworkday.macos"' ;;
