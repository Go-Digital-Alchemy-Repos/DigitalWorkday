...
export default function PlatformInvitePage() {
  ...
  const { data: inviteData, isLoading: verifying, error: verifyError } = useQuery<InviteVerifyResponse>({
    ...
    queryFn: async () => {
      ...
      if (!response.ok) {
        ...
        throw new Error(data.message || data.error?.message || "Failed to verify invite");
      }
  ...
  const acceptMutation = useMutation({
    mutationFn: async (data: { token: string; password: string }) => {
      const response = await apiRequest("POST", "/api/v1/auth/platform-invite/accept", data);
  ...
  if (verifyError) {
    const errorMessage = verifyError instanceof Error ? verifyError.message : "Failed to verify invite";
        ...
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="rounded-lg border border-muted p-4 bg-muted/20">
              ...
              {inviteData?.targetUser && (
                <div className="space-y-1 mt-3">
                  <div className="text-sm text-muted-foreground">Name</div>
                  <div className="font-medium">
                    {inviteData.targetUser.firstName} {inviteData.targetUser.lastName}
                  </div>
                </div>
              )}
            </div>
