...
function NewRequestDialog({
  ...
  const createMutation = useMutation({
    ...
    onSuccess: (data: { id: string }) => {
      queryClient.invalidateQueries({ queryKey: tenantKey(["/api/crm/portal/conversations"]) });
...
function ConversationThread({
  ...
  const { data, isLoading } = useQuery<ConversationDetail>({
    queryKey: tenantKey(["/api/crm/conversations", conversationId, "messages"]),
    queryFn: async () => {
  ...
  const sendMutation = useMutation({
    ...
    onSuccess: () => {
      setReplyText("");
      queryClient.invalidateQueries({ queryKey: tenantKey(["/api/crm/conversations", conversationId, "messages"]) });
      queryClient.invalidateQueries({ queryKey: tenantKey(["/api/crm/portal/conversations"]) });
    },
    onError: (error: Error) => {
      toast({ title: "Error", description: error.message, variant: "destructive" });
...
export default function ClientPortalMessages() {
  ...
  const { data: conversations = [], isLoading } = useQuery<ConversationSummary[]>({
    queryKey: tenantKey(["/api/crm/portal/conversations"]),
    enabled: crmFlags.clientMessaging,
  });

  const { data: currentUser } = useQuery<{ id: string }>({
    queryKey: tenantKey(["/api/auth/me"]),
    queryFn: async () => {
