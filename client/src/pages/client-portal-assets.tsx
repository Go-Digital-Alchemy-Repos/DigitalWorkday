...
import { Download, File as FileIcon, Folder, FolderOpen, FolderPlus, Upload } from "lucide-react";
...
export default function ClientPortalAssets() {
  ...
  const createFolder = useMutation<FolderRow, Error, string>({
    mutationFn: async (name) => (await apiRequest("POST", `/api/client-portal/clients/${clientId}/assets/folders`, {
      name,
      parentFolderId: folderId === "root" ? null : folderId,
    })).json(),
    onSuccess: (folder) => {
      setNewFolder("");
      queryClient.setQueryData<FolderRow[]>(queryKeys.portal.clientAssetFolders(clientId), (folders = []) => (
        folders.some((item) => item.id === folder.id) ? folders : [...folders, folder]
      ));
      queryClient.invalidateQueries({ queryKey: queryKeys.portal.clientAssetFolders(clientId) });
      toast({ title: "Folder created", description: `${folder.name} is ready for files.` });
    },
    onError: (error: Error) => toast({ title: "Unable to create folder", description: error.message, variant: "destructive" }),
  });
  ...
  const currentFolders = (foldersQuery.data ?? []).filter((folder) => folder.parentFolderId === (folderId === "root" ? null : folderId));
  ...
  return <div className="p-3 sm:p-6 overflow-y-auto h-full space-y-5">
    ...
    <Card><CardContent className="p-4 flex flex-col sm:flex-row gap-3"><Select value={folderId} onValueChange={setFolderId}><SelectTrigger className="sm:w-64"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="root">Root</SelectItem>{foldersQuery.data?.map((folder) => <SelectItem key={folder.id} value={folder.id}>{folder.name}</SelectItem>)}</SelectContent></Select>{folderId !== "root" && <Button variant="outline" onClick={() => { const current = foldersQuery.data?.find((folder) => folder.id === folderId); const name = window.prompt("Folder name", current?.name || ""); if (name?.trim()) renameFolder.mutate(name.trim()); }}>Rename folder</Button>}<Input placeholder="New folder name" value={newFolder} onChange={(event) => setNewFolder(event.target.value)} /><Button variant="outline" onClick={() => createFolder.mutate(newFolder.trim())} disabled={!newFolder.trim() || createFolder.isPending}><FolderPlus className="h-4 w-4 mr-2" />{createFolder.isPending ? "Creating…" : "New folder"}</Button><input ref={inputRef} type="file" className="hidden" onChange={(event) => { const file = event.target.files?.[0]; if (file) upload.mutate(file); event.target.value = ""; }} /><Button onClick={() => inputRef.current?.click()} disabled={upload.isPending}><Upload className="h-4 w-4 mr-2" />Upload</Button></CardContent></Card>
    {!!currentFolders.length && <section><h2 className="mb-2 text-xs font-medium uppercase tracking-wider text-muted-foreground">Folders</h2><div className="grid grid-cols-1 gap-2 sm:grid-cols-2 lg:grid-cols-3">{currentFolders.map((folder) => <Card key={folder.id} className="flex cursor-pointer items-center gap-2 p-3 transition-colors hover:bg-muted/50" onClick={() => setFolderId(folder.id)} data-testid={`portal-folder-card-${folder.id}`}><FolderOpen className="h-5 w-5 shrink-0 text-primary" /><span className="truncate text-sm font-medium">{folder.name}</span></Card>)}</div></section>}
    {!!assetsQuery.data?.items.length && <section><h2 className="mb-2 text-xs font-medium uppercase tracking-wider text-muted-foreground">Files</h2><div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">{assetsQuery.data.items.map((asset) => <Card key={asset.id}><CardHeader className="pb-2"><CardTitle className="text-base flex items-center gap-2"><FileIcon className="h-4 w-4" />{asset.title}</CardTitle></CardHeader><CardContent><p className="text-xs text-muted-foreground">{asset.mimeType} · {Math.ceil(asset.sizeBytes / 1024)} KB</p>{asset.description && <p className="text-sm mt-2">{asset.description}</p>}<div className="flex flex-wrap gap-2 mt-3"><Button variant="outline" size="sm" onClick={() => download(asset)}><Download className="h-4 w-4 mr-2" />Download</Button><Button variant="outline" size="sm" onClick={() => { const title = window.prompt("File name", asset.title); if (title?.trim()) updateAsset.mutate({ assetId: asset.id, body: { title: title.trim() } }); }}>Rename</Button><Select value={asset.folderId || "root"} onValueChange={(target) => updateAsset.mutate({ assetId: asset.id, body: { folderId: target === "root" ? null : target } })}><SelectTrigger className="h-9 w-36"><SelectValue placeholder="Move" /></SelectTrigger><SelectContent><SelectItem value="root">Root</SelectItem>{foldersQuery.data?.map((folder) => <SelectItem key={folder.id} value={folder.id}>{folder.name}</SelectItem>)}</SelectContent></Select></div></CardContent></Card>)}</div></section>}
    {!currentFolders.length && !assetsQuery.data?.items.length && <div className="py-16 text-center text-muted-foreground"><Folder className="h-12 w-12 mx-auto mb-3 opacity-40" /><p>This folder is empty.</p><p className="mt-1 text-xs">Upload files or create folders to get started.</p></div>}
